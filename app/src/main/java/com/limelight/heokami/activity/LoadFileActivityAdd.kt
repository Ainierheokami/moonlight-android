...
class LoadFileActivityAdd : AppCompatActivity() {
        ...
        filePicker.pickFile(mimeType = "text/*", callback = object :
            ...
            override fun onCallBack(fileName: String, content: String, uri: Uri) {
                try {
                    ...
                    Toast.makeText(this@LoadFileActivityAdd, "追加配置成功，应用即将自动重启...", Toast.LENGTH_SHORT).show()
                    // 延迟 800ms 后自动重启应用，确保 SP 写入完成
                    restartApp()
                } catch (e: Exception) {
                    Log.e("pickFile", "追加按键配置异常", e)
                    Toast.makeText(this@LoadFileActivityAdd, "配置格式破损，无法追加虚拟键盘布局", Toast.LENGTH_LONG).show()
     ...
     * 重新启动主界面 PcView，使所有追加导入的虚拟键盘配置即时生效。
     */
    private fun restartApp() {
        Handler(Looper.getMainLooper()).postDelayed({
            val intent = Intent(this@LoadFileActivityAdd, PcView::class.java)
