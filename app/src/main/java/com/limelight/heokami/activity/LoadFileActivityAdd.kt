        ...
        filePicker.pickFile(mimeType = "text/*", callback = object :
            ...
            override fun onCallBack(fileName: String, content: String, uri: Uri) {
                try {
                    ...
                    android.widget.Toast.makeText(this@LoadFileActivityAdd, "加载配置成功", android.widget.Toast.LENGTH_SHORT).show()
                } catch (e: Exception) {
                    Log.e("pickFile", "追加按键配置异常", e)
                    android.widget.Toast.makeText(this@LoadFileActivityAdd, "配置格式破损，无法追加虚拟键盘布局", android.widget.Toast.LENGTH_LONG).show()
                }
