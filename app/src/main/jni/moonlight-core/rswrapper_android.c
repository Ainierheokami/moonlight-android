/**
 * @file rswrapper_android.c
 * @brief Android-safe wrappers for nanors vectorization with different ISA options
 */

// _FORTIFY_SOURCE can cause some versions of GCC to try to inline
...
// Include assert.h before entering Clang target attribute regions below.
// Android's assert declarations are not decorated for target multiversioning.
#include <assert.h>
...
#if !defined(__ANDROID__) && (defined(__x86_64__) || defined(__i386__) || (defined(_MSC_VER) && (defined(_M_IX86) || defined(_M_AMD64))))
